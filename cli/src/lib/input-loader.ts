...
export async function loadInputsFromYaml(
  ...
  const providerOptions = buildProducerOptionsFromBlueprint(blueprint, modelSelections, allowAmbiguousDefault);
  const selectionEntries = collectSelectionEntries(modelSelections);
  const syntheticInputs = [
    ...collectProducerScopedInputs(blueprint, providerOptions),
    ...selectionEntries,
  ];
  ...
  return { values, modelSelections, providerOptions };
}
...
function extractSchemaPropertyPaths(schemaText: string): string[] {
  let schema: unknown;
  try {
    schema = JSON.parse(schemaText);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid inputSchema JSON: ${message}`);
  }
  const paths: string[] = [];
...
function collectProducerScopedInputs(
  ...
  const visit = (node: BlueprintTreeNode) => {
    for (const producer of node.document.producers) {
      ...
      const addFlattenedConfig = (variant: ProducerModelVariant) => {
        ...
        if (variant.systemPrompt) addEntry(namespacePath, producerName, 'systemPrompt');
        if (variant.userPrompt) addEntry(namespacePath, producerName, 'userPrompt');
        if (variant.variables) addEntry(namespacePath, producerName, 'variables');
  ...
  visit(tree);
  // Include config properties from model input schemas so saved inputs with schema-backed keys remain valid.
  const processed = new Set<string>();
  for (const [key, options] of providerOptions) {
    const primary = options?.[0];
    if (!primary) continue;
    const sourcePath = primary.sourcePath ?? key;
    if (processed.has(sourcePath)) continue;
    processed.add(sourcePath);
    const schemaText = primary.inputSchema;
    if (!schemaText) continue;
    const { namespacePath, producerName } = parseQualifiedProducerName(sourcePath);
    for (const path of primary.configInputPaths ?? []) {
      addEntry(namespacePath, producerName, path);
    }
    for (const path of extractSchemaPropertyPaths(schemaText)) {
      addEntry(namespacePath, producerName, path);
    }
  }
