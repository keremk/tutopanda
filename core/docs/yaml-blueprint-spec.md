...
## 1. Repository layout
- **Top-level blueprints** live in `cli/blueprints/*.yaml`. Each file describes a public workflow the CLI can execute (audio-only, image-only, image+audio, etc.).
- **Modules** live under `cli/blueprints/modules/`. A module contains reusable node definitions (inputs, artefacts, producers) and can itself import other modules.
- **Prompt definitions** for LLM producers live in `cli/blueprints/modules/prompts/*.toml`.
- **JSON schemas** referenced by those prompts live in `cli/blueprints/modules/schemas/*.json`.
